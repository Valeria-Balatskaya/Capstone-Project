#!/usr/bin/env python3
"""
Mac Tag (Sender) Script - BACKUP OPTION
========================================
USE windows_tag.py INSTEAD! This is for backup if Windows tag doesn't work.

The normal 4-laptop setup uses windows_tag.py on the mobile Windows laptop.
This script is a fallback option if you need to run the tag on Mac instead.

Setup (only if windows_tag.py fails):
    1. Connect sender board to Mac USB (different port than receiver!)
    2. Find port: python3 -c "from serial.tools import list_ports; [print(p.device) for p in list_ports.comports()]"
    3. Run: python3 mac_tag.py --port /dev/cu.usbserial-XXXX
